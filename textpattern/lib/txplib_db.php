...
class DB
{
    ...
    public function __construct()
    {
        ...
        // Kick MLP off.
        if (!$mlp) {
            $mlp = new \Netcarver\MLP\Kickstart();
        }
    }
}
...
function safe_pfx($table)
{
    ...
    $table = $mlp->safe_remap_tables($table);
...
function safe_field($thing, $table, $where, $debug = false)
{
    ...
    if (@mysqli_num_rows($r) > 0) {
        ...
        $row = $mlp->safe_process_results($thing, $table, $where, $row);
...
function safe_row($things, $table, $where, $debug = false)
{
    ...
    $things = $mlp->safe_remap_fields($things, $table);
    $q = "SELECT $things FROM ".safe_pfx_j($table)." WHERE $where";
    $rs = getRow($q, $debug);

    if ($rs) {
        $rs = $mlp->safe_process_results($things, $table, $where, $rs);
        return $rs;
    }
...
function fetch($col, $table, $key, $val, $debug = false)
{
    ...
    $col = $mlp->safe_remap_fields($col, $table);
    $key = doSlash($key);
    ...
    if ($r = safe_query($q, $debug)) {
        if (mysqli_num_rows($r) > 0) {
            $row = mysqli_fetch_row($r);
            mysqli_free_result($r);
            $row = $mlp->safe_process_results($col, $table, array($key, $val), $row);
            return $row[0];
        }
